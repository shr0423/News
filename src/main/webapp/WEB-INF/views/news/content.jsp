...
class Row{
	constructor(msg,cwriter, cregdate){
		//js는 멤버변수를 constructor()인 생성자 안에서만 정의할 수 있다.
		...
		//생성된 this.div를 reply_area영역에 동적으로 자식요소로 추가
	...
	function getCommentsList(){
		...
		$.ajax({
			url:"/comments/list?news_idx=<%=news.getNews_idx()%>",
			type:"GET",
			success:function(result, status, xhr){//서버가 200(성공)으로 응답할 경우 success동작함
				console.log("서버로부터 받은 결과는 ",result);
			
				//기존에 reply_area에 추가된 자식들을 싹 지우고 for문 실행
				
				$("#reply_area").empty();//모든 자식요소 삭제
	...
	$(function() {
		...
		$("#bt_regist").click(function() {
			//비동기방식으로 서버에 글 등록요청을 하겟음
			$.ajax({
				url:"/comments/regist",
				type:"post",
				data:{
					"news.news_idx":<%=news.getNews_idx()%>,
					"msg":$("input[name='msg']").val(),
					"cwriter":$("input[name='cwriter']").val()
				},
				success:function(result,status, xhr){
					console.log("등록 후 서버의 응답", result);
					//서버가 보낸 메시지가 ok라면 곧바로 비동기 방식으로 서버로부터 list를 가져오기
					getCommentsList();
				}
...
<body>
	...
	<div class="container">
		...
		<div>
			<input type="text" name="msg" placeholder="댓글 내용 작성"
				style="width: 70%"> <input type="text" name="cwriter"
				placeholder="작성자" style="width: 20%"> <input type="button"
				value="등록" id="bt_regist" style="width: 8%">
		</div>
